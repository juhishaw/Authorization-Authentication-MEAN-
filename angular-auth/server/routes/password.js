...
router.post("/forgot_password", async (request, response) => {
  ...
  if (user) {
    ...
    forgotPasswordResponse = {
      data: { email: user.email, passwordToken: user.resetPasswordToken },
      message: "Plase check youe email or spam",
      status: 200
    };
...
router.post("/reset_password", async (request, response) => {
  ...
  if (user) {
    ...
    response
      .status(400)
      .send("This password reset link is either expired or invalid");
  }
});
